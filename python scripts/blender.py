...
class PoseEstimator:
    ...
    def get_full_model_points(self, filename='/media/ubuntu/data/tool/blender-2.82-linux64/pure/model.txt'):
        ...
        print("Createing Detector............")
        detector = Face_Track.getDetector()

        print("capturing Vedio...............")
        capture = cv2.VideoCapture(0)

        for eachgray in Face_Track.getGrayFrame(capture):
            # 分离单人与多人
            # 多张人脸会卡住
            for face_landmarks in Face_Track.facial_landmark_detection(detector, eachgray):
                print(eachgray)
                model_points = np.reshape(Face_Track.rawdataWithoutColor(face_landmarks), (-1, 3))
                # 只有一 frame
                return model_points
        # 不会释放
        capture.release()
...
class FacialFeatures:
    ...
    def eye_aspect_ratio(image_points, side):
        ...
        ear = ear * (np.linalg.norm(tip_of_eyebrow - image_points[2]) / np.linalg.norm(
    ...
    def modal(self, context, event):
        ...
        if event.type == 'TIMER':
            ...
            if faces:
                ...
                x_ratio_left, y_ratio_left = FacialFeatures.detect_iris(self.image_points, self.iris_image_points,
                                                                        Eyes.LEFT)
                x_ratio_right, y_ratio_right = FacialFeatures.detect_iris(self.image_points, self.iris_image_points,
                                                                          Eyes.RIGHT)
